...
function deriveIsVerified(user) {
  if (user.role === "STUDENT")
    return user.studentVerification?.status === "VERIFIED";
  if (user.role === "LANDLORD")
    return user.landlordVerification?.status === "VERIFIED";
...
const authService = {
  async register({
    ...
    if (role === "STUDENT") {
      ...
      logger.info("Verification code generated and email sent", {
        userId: user.id,
      });
    }
    ...
    if (role === "LANDLORD") {
      ...
      logger.info(
        "LandlordVerification record created (awaiting document upload)",
        { userId: user.id },
      );
    }
  ...
  async googleAuth({ idToken }) {
    ...
    if (!user) {
      ...
      logger.info("Google OAuth — existing user signed in", {
        userId: user.id,
        email,
      });
    }
  ...
  async verifyEmail({ userId, code }) {
    ...
    if (!user || user.role !== "STUDENT")
      throw new ForbiddenError("Only students can verify email");
  ...
  async resendCode({ userId }) {
    const user = await userRepository.findById(userId);
    if (!user || user.role !== "STUDENT")
      throw new ForbiddenError("Only students can request verification codes");
