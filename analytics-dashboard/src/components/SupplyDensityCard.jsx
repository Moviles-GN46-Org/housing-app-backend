  ...
  if (error) {
    return (
      <div className="bg-white_card rounded-2xl border border-[#E8DDD4] p-12 flex flex-col items-center justify-center text-center">
        ...
        <p className="text-taupe text-xs">Check that the backend is running at {API_URL}</p>
      </div>
    );
  }
      ...
      <div className="px-6 pt-6 pb-5">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-taupe text-xs font-medium uppercase tracking-wider">Supply Density</p>
              <h2 className="text-mocha font-semibold text-sm">Are there enough properties nearby?</h2>
            </div>
          </div>
          <div className={`flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full ${
            data?.status === 'High Supply' ? 'bg-emerald-50 text-emerald-600' : 'bg-orange-50 text-orange-600'
          }`}>
            <Activity size={11} />
        ...
        <div className="mt-6 bg-linen rounded-2xl p-6 border border-[#E8DDD4] flex items-center justify-between">
          <div>
            <p className="text-taupe text-xs mb-1 uppercase font-bold tracking-tighter">Global Average</p>
            <p className="text-mocha font-bold text-5xl tracking-tighter">
              {(data?.averageDensity * 100).toFixed(1)}%
            </p>
          </div>
          <div className="text-right">
            <p className="text-mocha font-bold text-lg">{data?.totalChecks || 0}</p>
            <p className="text-taupe text-[10px] leading-tight">GPS Checks<br/>performed today</p>
          </div>
        </div>

        <div className="mt-6">
          <p className="text-mocha font-medium text-xs mb-2">Availability Analysis</p>
          <div className="h-2 bg-[#F0E8E0] rounded-full overflow-hidden">
            <div 
              className="h-full bg-bronze transition-all duration-1000"
              style={{ width: `${(data?.averageDensity * 100)}%` }}
            ></div>
          </div>
          <p className="text-taupe text-[11px] mt-3 italic leading-snug">
            * This metric represents the percentage of total inventory properties found within a 5km radius by users.
          </p>
        </div>
      </div>
